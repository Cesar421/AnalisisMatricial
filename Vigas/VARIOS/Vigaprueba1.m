    ...
    if cargasinternas > 0
        for p=1:1:cargasinternas;
            ...
            elseif Tipodecarga == 2
                ...
                Mempotra = (Puntual*Distanciaizq*Distanciader^2)/Long^2;
                %Buscar el cortante de empotramiento
                %perfecto-------------------------------------------------------------------------------
                Vempotra = 1;
            end
            %Aqui se suma los momentos y cargas de empotramiento a cada
            %grado de libertad de los vectores FN y FA de la estructura
            if 
            FN(Elem(i,1),1) = FN(Elem(i,1),1) + Mempotra;
            FN(Elem(i,2),1) = FN(Elem(i,2),1) - Mempotra;
            else
            FA(Elem(i,1),1) = FA(Elem(i,1),1) + Mempotra;
            end
            %---------------------------------------------------------------------------------------------------
        end
    end
