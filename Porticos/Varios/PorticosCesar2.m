...
for l = 1:1:numerodeelemcaragdos;
    ...
    for i = 1:1:cargasinternas;
        ...
        Vfglobaparavectorruerzas(:,1,queelemento) = vfglob(:,1,queelemento);
        disp ('Matriz Local');
        disp (vfloc);
        disp ('Matriz Global');
        disp (vfglob);
        ...
        for k=1:1:6;
            Femporamiento(F(k)) = Femporamiento(F(k))+ Vfglobaparavectorruerzas(k,1,queelemento);
        end
        Vfglobaparavectorruerzas = zeros(6,1);
        disp (Femporamiento);
    end
end
