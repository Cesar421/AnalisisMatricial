...
vfloc = zeros (6,1,nel);
ndesplazconocidosapoyos=zeros(nnd,3); 

%% Entrada de datos de los nudos
for i=1:1:nnd;
    nudo=i;
    Coord(i,1)=input(['Coordenada x del nudo (m) ' num2str(i) '\n']);
    Coord(i,2)=input(['Coordenada y del nudo (m) ' num2str(i) '\n']);  
end
...
for cont1=1:1:ndesplazacono
    ...
    j=input(['Dirección de desplazamiento en x=1, y=2 , Giro = 3 ' '\n']);
    if j<3
...
for cont1=1:1:nFext
    ...
    if j<3
     Fext(i,j)=input(['Magnitud y sentido de la carga en el nudo (KN,KN-m' num2str(i) '\n']);
    else 
...
for i=1:1:nsec;
    Base = input(['Base del elemento (m) ' num2str(i) '\n']);
    Altura = input(['Altura del elemento (m) ' num2str(i) '\n']);
...
for i=1:1:nmat;
    Mat(i,1)=input(['Módulo de elasticidad del material? (Mpa)' num2str(i) '\n'])*10^6;
end
...
for i=1:1:nel
    ...
    Delx(i)=xf-xi;
    Dely(i)=yf-yi;
    Elem(i,5)=sqrt(Delx(i)^2+Dely(i)^2);
    disp (Elem);
end
...
u = reshape(Fext',1,GL);
cont =0;
%% Creacion matriz de fuerzas externas
for i=1:1:GL
        if f(1,i) == 0    
            cont = 1+cont;
...
%% Creacion vectod de desplazamientos
f = reshape(CAp',1,GL);
...
%% Calculo de desplazamientos desconocidos y fuerzas desconocidas.
Desplazamiento = inv(KT(1:GIC,1:GIC))* ((Mfuezext-Femporamiento(1:GIC,1))-(KT(1:GIC,GIC+1:GL)*Vdesplacono));

Fuerzasdescon = KT(GIC+1:GL,1:GIC)*Desplazamiento+Femporamiento(GIC+1:GL,1)+(KT(GIC+1:GL,GIC+1:GL)*Vdesplacono);
Desplaztotal = zeros(GL,1);

Desplaztotal(1:GIC,1)=Desplaztotal(1:GIC,1)+Desplazamiento(1:GIC,1);
...
for i=1:1:nel
    FLocelem(:,1,i)= T(:,:,i)'*FGlobelem(:,1,i);      
end
