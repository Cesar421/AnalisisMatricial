...
for l = 1:1:numerodeelemcaragdos;
    ...
    for i = 1:1:cargasinternas;
        if Elementoinclinadopregunta == 1;
            proyeccionausar = input (['Que proyeccion va a usar X[1] o Y[2] en el elemento ' num2str(queelemento) '\n' ]);
        ...
        elseif Tipodecarga == 3;
            Momentointerno = input (['¿ Magnitud del Momento en (KN-m) en el elemento ' num2str(queelemento) '? ''\n']);
            Distanciaizq = input (['¿ Distancia de izquierda a derecha ?' '\n']);
            Distanciader = L - Distanciaizq ;
            vfglob(:,1,queelemento)=[0;
        ...
        F(2) = MGL(Elem(queelemento,1),2);
        F(3) = MGL(Elem(queelemento,1),3);
        F(4) = MGL(Elem(queelemento,2),1);
        F(5) = MGL(Elem(queelemento,2),2);
        F(6) = MGL(Elem(queelemento,2),3);
        for k=1:1:6;
            Femporamiento(F(k)) = Femporamiento(F(k))+ vfglob(k,1,queelemento);
        end
        disp (Femporamiento);
    end
end
