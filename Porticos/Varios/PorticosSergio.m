...
for l = 1:1:numerodeelemcaragdos;
    ...
    if inclelem == 1;
        ...
        for i = 1:1:cargasinternas1;            
                ...
                vfglob(:,1,queelemento)=[0;-(w*L)/2;-(w*L^2)/12;0;-(w*L)/2;(w*L^2)/12]+vfglob(:,1,queelemento);
                ...
                vfglob(:,1,queelemento)=[-(w*L)/2;0;0;-(w*L)/2;0;0]+vfglob(:,1,queelemento);;
                vfloc(:,1,queelemento)= Matrizdetransformacion(:,:,queelemento)*vfglob(:,1,queelemento);
                elseif Tipodecarga == 4;
                Puntual = input (['¿Magnitud y sentido de la carga axial puntual (kN)? ']);
                Distanciaizq = input (['¿Distancia de izquierda a derecha de la carga? ']);
                Distanciader = input (['¿Distancia derecha izquierda de la carga? ']);
                vfglob(:,1,queelemento)=[-(Puntual*Distanciader)/L;0;0;-(Puntual*Distanciaizq)/L;0;0]+vfglob(:,1,queelemento);;
                vfloc(:,1,queelemento)= Matrizdetransformacion(:,:,queelemento)*vfglob(:,1,queelemento);
                end
                    ...
                    for i = 1:1:cargasinternas2;
                        ...
                        if Tipodecarga == 1;
                            ...
                            vfglob(:,1,queelemento)=[0;
                                ...
                                0;
                                -(w*Elem(queelemento,5))/2;
                                (w*Elem(queelemento,5)^2)/12]+vfglob(:,1,queelemento);
                        ...
                        elseif Tipodecarga == 2;
                            ...
                            Distanciader = input (['¿Distancia derecha izquierda de la carga? ']);
                            vfglob(:,1,queelemento)=[0;
                                -((3*Distanciaizq)+(Distanciader))*((Puntual*Distanciader^2)/Elem(queelemento,5)^3);
                                -(Puntual*Distanciaizq*Distanciader^2)/Elem(queelemento,5)^2;
                                0;
                                -((Distanciaizq)+(3*Distanciader))*((Puntual*Distanciaizq^2)/Elem(queelemento,5)^3);
                                (Puntual*Distanciaizq^2*Distanciader)/Elem(queelemento,5)^2]+vfglob(:,1,queelemento);
                        
                            vfloc(:,1,queelemento)= Matrizdetransformacion(:,:,queelemento)*vfglob(:,1,queelemento);
                            
                        elseif Tipodecarga == 3;
                            w = input(['¿ Magnitud y sentido de la carga axial distribuida (kN/m)? ']);
                            vfglob(:,1,queelemento)=[-(w*Elem(queelemento,5))/2;
                                    0;
                                    0;
                                    -(w*Elem(queelemento,5))/2;
                                    0;
                                    0]+vfglob(:,1,queelemento);;
                        ...
                        elseif Tipodecarga == 4;
                            ...
                            Distanciaizq = input (['¿Distancia de izquierda a derecha de la carga? ']);
                            Distanciader = input (['¿Distancia derecha izquierda de la carga? ']);
                            vfglob(:,1,queelemento)=[-(Puntual*Distanciader)/Elem(queelemento,5);
                                                       0;
                                                       0;
                                                     -(Puntual*Distanciaizq)/Elem(queelemento,5);
                                                       0;
                                                       0]+vfglob(:,1,queelemento);;
                            vfloc(:,1,queelemento)= Matrizdetransformacion(:,:,queelemento)*vfglob(:,1,queelemento);                            
                        end
