...
for cont1=1:1:nFext
    ...
    j=input(['Dirección de la carga en Mx=1, My=2, z=3' '\n']);
    if j<4
...
tiposec = input(['Sección rectangular [1] o circular [2] ' num2str(i) '\n']);
for i=1:1:nsec;
    if tiposec == 1;
        Base = input(['Base de la sección ' num2str(i) '\n']);
        Altura = input(['Altura de la sección ' num2str(i) '\n']);
        Secc(i,1)= Base*Altura;
        Secc(i,2)=Base*Altura^3/12;
        if Base<Altura;
            Secc(i,3)=((1/3)-((0.21*Base/Altura)*(1-((Base/Altura)^4/12))))*Altura*(Base^3);
        else
            Secc(i,3)=((1/3)-((0.21*Altura/Base)*(1-((Altura/Base)^4/12))))*Base*(Altura^3);
        end       
    elseif tiposec == 2;
        Diam = input(['Diámetro de la sección ' num2str(i) '\n']);
        Secc(i,3)= pi*(Diam^4)/32;              
    end
end
    ...
    for i = 1:1:cargasinternas;
        ...
        elseif Tipodecarga == 2;
            ...
            vfloc(:,1,queelemento)=[0;
                                    ...
                                    -Puntual*(Distanciaizq^2/L^2)*(3-2*(Distanciaizq/L))]+ vfloc(:,1,queelemento);
            vfglob(:,1,queelemento) = Matrizdetransformacion(:,:,queelemento)*vfloc(:,1,queelemento);            
        elseif Tipodecarga == 3;
            ...
            vfloc(:,1,queelemento)=[0;
                                    ...
                                    -Momentointerno*6*Distanciaizq*Distanciader/L^3] + vfloc(:,1,queelemento);           
            vfglob(:,1,queelemento) = Matrizdetransformacion(:,:,queelemento)*vfloc(:,1,queelemento);            
        elseif Tipodecarga == 4;
            % La carag simepre se asume que va a la derecha es decir ejel
            % local X positivo.
            Axialpunt= input (['¿ Magnitud del mommento torsor axial Puntual (KN) en el elemento ' num2str(queelemento) '? ''\n']);
            Distanciaizq = input (['¿ Distancia de izquierda a derecha ?' '\n']);
        ...
        elseif Tipodecarga == 5;
            % La carag simepre se asume que va a la derecha es decir ejel
            % local X positivo.
            Axidist = input (['¿ Magnitud del momento torsor axial distribuido (KN/m) en el elemento ' num2str(queelemento) '? ''\n']);
...
for i=1:1:nel
    ...
    GJ=(Mat(Elem(i,4),1)/(2*(1+Poisson)))*Secc(Elem(i,3),3);
...
% Esto se hace para poder usar la matri
f = reshape(CAp',1,GL);
u = reshape(Fext',1,GL);
...
Desplazamiento = inv(KT(1:GIC,1:GIC))* (Mfuezext(1:GIC,1)-Femporamiento(1:GIC,1));
